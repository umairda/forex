...
router.get('/:pair', function(req, res, next) {
	Ohlc.find({instrument:req.params.pair}, {_id:0,instrument:0},function(err,ohlcs) {
		res.json(ohlcs);
	});	
});

router.get('/:pair/:sdate/:edate', function(req, res, next) {
	Ohlc.find({instrument:req.params.pair}, {_id:0,instrument:0})
...
router.get('/:pair/:field', function(req, res, next) {
	...
	if (typeof Ohlc.schema.paths[field]!='undefined')
	{
		...
		fields[field]=1;
		Ohlc.find({instrument:req.params.pair}, fields, function(err,ohlcs) {
